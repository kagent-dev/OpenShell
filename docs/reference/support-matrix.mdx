...
OpenShell publishes three container images. All are published for `linux/amd64` and `linux/arm64`.
...
| Supervisor | `ghcr.io/nvidia/openshell/supervisor:latest` | Sandbox pod init container startup |

The cluster image bundles k3s, Helm charts, Kubernetes manifests, and bootstrap scripts. The gateway image is pulled at cluster startup and runs the API server. The supervisor image is pulled by sandbox pod init containers and copies `/openshell-sandbox` into a shared volume before the agent container starts.
