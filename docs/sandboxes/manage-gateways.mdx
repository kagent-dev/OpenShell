...
The gateway usually runs inside a Docker container and exposes a single port (gRPC and HTTP multiplexed), secured by mTLS by default. You can also install the same gateway chart into an existing Kubernetes cluster. The gateway can be deployed locally, on a remote host via SSH, in an operator-managed Kubernetes cluster, or behind a cloud reverse proxy.
...
Install the Helm chart into an existing cluster when you want the gateway to run in-cluster and manage sandbox pods with its ServiceAccount.

Create a values file for the minimum plaintext deployment:
...
For local development, reach the gateway with port forwarding:
...
For shared use, expose the Service through `NodePort`, `LoadBalancer`, or `Ingress`, then register the externally reachable URL:

```shell
openshell gateway add http://<reachable-host>:<port>
```

The chart installs the required ServiceAccount and RBAC automatically. The current minimum permissions are sandbox CRD access in the sandbox namespace, namespaced event reads, and a cluster-scoped read-only `nodes` permission for GPU capacity checks.
